...
| Web stack | Writable forum upload path (`hub_storage_path('uploads/forum')`) | Yes | Default host path: `/var/khubdata/files/uploads/forum/` — see [../deployment/STORAGE.md](../deployment/STORAGE.md) |
...
- The user running **PHP-FPM**, **Apache** `mod_php`, or **php-cgi** must:
  ...
  - **Read** uploaded source files and **write** PDFs under the configured hub files root (`uploads/forum/`)
...
1. File is validated and moved to `{HUB_FILES_ROOT}/uploads/forum/{md5}.{ext}` (via `hub_storage_path()`).
...
- [../deployment/STORAGE.md](../deployment/STORAGE.md) — host paths, symlinks, `/hub-media` URLs
- [../deployment/PERMISSIONS.md](../deployment/PERMISSIONS.md) — writable paths and `fix-storage-permissions.sh`
