...
if (document.getElementById("_dm-settingsContainer")) {
   ...
   const tryTransitionBtn = document.getElementById("_dm-tryTransitions");

   transitionSelect.addEventListener("change", (e) => {
      const _this = e.target;

      if (tryTransitionBtn.classList.contains("d-none")) tryTransitionBtn.classList.remove("d-none");
   ...
   // RTL Switcher
   // ---------------------------------------------------------------------------------
   const rtlBtn = document.getElementById("_dm-rtlBtn");
   let siteDirection = document.documentElement.getAttribute("dir");

   rtlBtn.addEventListener("click", () =>{
      if ( siteDirection == "rtl" ) {
         document.documentElement.setAttribute("dir", "ltr");
         rtlBtn.textContent = "Switch to RTL";
      } else {
         document.documentElement.setAttribute("dir", "rtl");
         rtlBtn.textContent = "Switch to LTR";
      }

      siteDirection = document.documentElement.getAttribute("dir");
   })
   ...
   const setRangeValue = ( range, bubble, preffix, suffix ) => {
      ...
      bubble.style.insetInlineStart = `calc(${newVal}% + (${8 - newVal * 0.15}px))`;
   }
