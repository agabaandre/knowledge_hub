...
@if(!empty($aiPage['web_search_providers']))
<div class="learning-section-card">
    <div class="section-head">
        <h4><i class="fa fa-globe me-2" style="color: var(--la-primary);"></i>{{ __('admin_nav.ai_web_search') }}</h4>
        <p>{{ __('admin_nav.ai_web_search_intro') }}</p>
    </div>
    <div class="section-body">
        @foreach($aiPage['web_search_providers'] as $provider)
            <div class="platform-card">
                <div class="platform-card-head">
                    <div class="d-flex align-items-start">
                        <div class="provider-icon" style="background: {{ $provider['color'] }};">
                ...
                <div class="px-3 pb-2 border-bottom">
                    <div class="row g-2 align-items-end">
                        <div class="col-md-4 source-priority-wrap">
                            <label class="form-label mb-1" for="ai_source_priority_{{ $provider['id'] }}">{{ __('admin_nav.ai_source_priority') }}</label>
                            <select name="ai_source_priority[{{ $provider['id'] }}]" id="ai_source_priority_{{ $provider['id'] }}" class="form-select form-select-sm">
                                <option value="env" @selected(($provider['source_priority'] ?? 'env') === 'env')>{{ __('admin_nav.ai_source_env') }}</option>
                                <option value="db" @selected(($provider['source_priority'] ?? 'env') === 'db')>{{ __('admin_nav.ai_source_db') }}</option>
                            </select>
                        </div>
                    </div>
                </div>
                <div class="px-3 pb-3 pt-3">
                    @if($provider['id'] === 'serper')
                        <div class="row g-3">
                            <div class="col-md-8">
                                <label class="form-label" for="ai_serper_api_key">{{ __('admin_nav.ai_serper_api_key') }}</label>
                                <input type="password" name="ai_serper_api_key" id="ai_serper_api_key" class="form-control form-control-sm" autocomplete="new-password"
                                       placeholder="{{ !empty($fields['ai_serper_api_key']['value']) ? '••••••••' : __('admin_nav.ai_serper_api_key_placeholder') }}">
                                <small class="text-muted">{{ __('admin_nav.ai_serper_env_hint') }} <code>SERPER_API_KEY</code></small>
                            </div>
                        </div>
                    @endif
                </div>
